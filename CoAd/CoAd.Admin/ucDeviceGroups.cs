...
{
    public partial class ucDeviceGroups : UserControl
    {
        public ucDeviceGroups()
        {
            InitializeComponent();
        ...
        private void lueServers_EditValueChanged(object sender, EventArgs e)
        {
            ...
            //Отображаем только клиентские группы
            gcDeviceGroups.DataSource = Base.db.ClientDeviceGroup.Where(dg => dg.id_server == CurrentServer.id && dg.fo_id_group.HasValue);
        }
        ...
        private void LoadCurrentGroup()
        {
            ...
            {
                labelId.Text = string.Format("Идентификатор группы: {0}", "");
            ...
            labelId.Text = string.Format("Идентификатор группы: {0}", CurrentDeviceGroupRow.fo_id_group);
        ...
        public void AddEditorRow(DeviceGroupParams dr)
        {
            try
            {
                //Поведение будет зависеть от того, является ли группа подчиненной группе ЦАС
                var PrimaryGroup = Base.db.ClientDeviceGroup
                    .Join(Base.db.DeviceGroupDependency, dg => dg.id, gd => gd.id_primary_group, (gd, dg) => new { primaryGroup = gd, depednency = dg })
                    .Where(dg => dg.depednency.id_secondary_group == dr.id_group).FirstOrDefault();
                DeviceGroupParams PrimaryGroupParam = null;
                if (PrimaryGroup != null) PrimaryGroupParam = Base.db.DeviceGroupParams.FirstOrDefault(p => p.id_group == PrimaryGroup.primaryGroup.id && p.id_type == dr.id_type);
                if (PrimaryGroupParam != null) dr.value = PrimaryGroupParam.value;
                ...
                EditorRow editorRow = new EditorRow(nameStrCashParam) { Name = nameStrCashParam, Enabled = PrimaryGroupParam == null };
                editorRow.Properties.FieldName = nameStrCashParam;
                editorRow.Properties.Caption = dr.TypeParams.name;
                if (PrimaryGroupParam != null) editorRow.Properties.ToolTip = string.Format("Управляется группой {0}", PrimaryGroup.primaryGroup.name);
